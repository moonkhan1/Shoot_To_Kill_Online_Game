...
public class NetworkCharacterControllerPrototypeCustom : NetworkTransform {
  ...
  /// </summary>
  public virtual void Move2(Vector3 direction) {
  ...
  public virtual void Move(Vector3 direction) {
    ...
    // Calculate the forward direction based on the character's rotation
    Vector3 forwardDirection = transform.forward;
    forwardDirection.y = 0f; // Keep it horizontal
    ...
    if (direction != default) {
      // Calculate the desired movement direction based on character orientation
      var desiredDirection = forwardDirection * direction.z + transform.right * direction.x;

      horizontalVel = Vector3.ClampMagnitude(desiredDirection * acceleration * deltaTime, maxSpeed);
    } else {
  ...
  public void Rotation(float rotationY)
  {
